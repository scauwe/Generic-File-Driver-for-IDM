...
public class RandomGUID extends Object {
    ...
    static {
        ...
        try {
        	...
        	try{
        		//In case of an exception, replace it with something else:
        		//elapsed time since starting the jvm
        		s_id=Long.toString(System.currentTimeMillis() - ManagementFactory.getRuntimeMXBean().getStartTime());
        	}catch (Exception e2) {
        		//Something went wrong with the management interface???
        		//Final fallback
        		s_id=Long.toString(System.currentTimeMillis() );
			}
        }
