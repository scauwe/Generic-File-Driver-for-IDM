...
public class Util {
	...
	/**
	 * Get the first statuslevel found in the given XmlDocument or null when no
	 * status level can be found.
	 *
	 * @param xmlDoc
	 * @return
	 */
	...
	/**
	 * Get the description of the first status level found in the given
	 * XmlDocument or null when no description is associated with the first
	 * status level.
	 *
	 * @param xmlDoc
	...
	/**
	 * Get the worst found status level from the XmlDocument (or SUCCESS when no
	 * status level can be found)
	 *
	 * @param xmlDoc
	 * @return
	...
	private static FileLock getFileLock(Trace trace, FileChannel fileChannel, int counter) {
		...
		if (lock == null) {
			...
			trace.trace("Failed to get write lock on file before copy. Will try again in 1 seconds",
	...
	/**
	 * Copy a file from 'from' to 'to'. Returns true if the file was copied.
	 * False otherwise.
	 *
	 * @param from
	 * @param to
	...
	private static boolean moveFileJava7(Trace trace, File sourceFile, File destFile) {
		try {
			// First try the Files.move as an atomic operation
			trace.trace("Moving file from " + sourceFile.getAbsolutePath() + " to " + destFile.getAbsolutePath(),
					TraceLevel.TRACE);
			...
			try {
				...
				trace.trace(
						"Non-atomic move from " + sourceFile.getAbsolutePath() + " to " + destFile.getAbsolutePath(),
						TraceLevel.TRACE);
				Files.move(sourceFile.toPath(), destFile.toPath(), StandardCopyOption.REPLACE_EXISTING);
				return true;
			} catch (Throwable e2) {
	...
	public static boolean moveFile(Trace trace, File sourceFile, File destFile) {
		...
		// return true;
		// } else {
		// trace.trace("Trying to move via copy-delete sequence.",
		// TraceLevel.TRACE);
		// if (copyFile(trace, sourceFile, destFile)) {
		// if (!sourceFile.delete()) {
		// trace.trace("File was copied, but failed to delete the original
		// file.", TraceLevel.ERROR_WARN);
		// }
		// trace.trace("Copy-delete sequence done.", TraceLevel.TRACE);
		// return true;
		// } else {
		// trace.trace("Failed to move or copy the file to the destiantion.",
		// TraceLevel.ERROR_WARN);
		// return false;
		// }
		// }
		/*
		 * Java 7 move not yet supported on all environments. try { // first try
		 * the renameTo method trace.trace("Moving file from "
		 * +sourceFile.getAbsolutePath()+" to "
		 * +destFile.getAbsolutePath(),TraceLevel.TRACE);
		 * Files.move(sourceFile.toPath(), destFile.toPath(),
		 * StandardCopyOption.REPLACE_EXISTING, StandardCopyOption.ATOMIC_MOVE);
		 * return true; }catch (Exception e) { try{ trace.trace(
		 * "Non-atomic move from "+sourceFile.getAbsolutePath()+" to "
		 * +destFile.getAbsolutePath(),TraceLevel.TRACE);
		 * Files.move(sourceFile.toPath(), destFile.toPath(),
		 * StandardCopyOption.REPLACE_EXISTING); return true; }catch (Exception
		 * e2) { Util.printStackTrace(trace, e2); return false; } }
		 */
		/*
		 * if (sourceFile.renameTo(destFile)) { trace.trace(
		 * "Moved via java native.",TraceLevel.TRACE); return true; } else {
		 * //assume linux. Call mv external trace.trace("Trying via unix: mv " +
		 * sourceFile.getAbsolutePath()+" "
		 * +destFile.getAbsolutePath(),TraceLevel.TRACE); Process p =
		 * Runtime.getRuntime().exec(new String[]{"/bin/sh","-c",
		 * "mv",sourceFile.getAbsolutePath(),destFile.getAbsolutePath()});
		 * trace.trace("Output from mv"); String line; BufferedReader input =
		 * new BufferedReader (new InputStreamReader(p.getInputStream())); while
		 * ((line = input.readLine()) != null) { System.out.println(line); }
		 * input.close(); trace.trace("Waiting for mv",TraceLevel.TRACE);
		 * p.waitFor();
		 *
		 * trace.trace("Return code: "+p.exitValue(),TraceLevel.TRACE); // try
		 * to copy file, delete original //copyFile(sourceFile, destFile, true,
		 * true); //sourceFile.delete(); } return true; } catch (IOException e)
		 * { trace.trace("IO Exception during moveFile "+sourceFile+" - "
		 * +destFile+" : "+ e.getMessage()); return false; } catch
		 * (InterruptedException e) { trace.trace(
		 * "InterruptedException during moveFile "+sourceFile+" - "+destFile+
	...
	/**
	 * Append the statusAttributes to the statusDoc and return the created
	 * statusElement
	 *
	 * @param statusDoc
	 * @param statusAttr
	 ...
	 * @return
	 */
	public static XDSStatusElement appendStatus(StatusDocument statusDoc, StatusAttributes statusAttr,
			String description) {
	 ...
	 */
	public static XDSStatusElement appendStatus(StatusDocument statusDoc, StatusAttributes statusAttr,
			String description, Throwable paramException, boolean printStackTrace, XmlDocument paramXmlDocument) {
		if (statusDoc != null) {
			...
			if ((paramException instanceof XDSParameterException)) {
				for (XDSParameterException localXDSParameterException = (XDSParameterException) paramException; localXDSParameterException != null; localXDSParameterException = localXDSParameterException
						.getNext()) {
	...
	public static XDSQueryDocument createQueryDoc(String className, String destDN, String association, QueryScope scope,
			Collection<String> readAttr, Map<String, String> matchAttributes) {
