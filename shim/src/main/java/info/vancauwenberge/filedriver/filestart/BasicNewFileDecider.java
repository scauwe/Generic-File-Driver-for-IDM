...
public class BasicNewFileDecider extends AbstractStrategy
		implements IFileStartStrategy, IShutdown, ISubscriberFileListener {
	/**
	 * positive seconds assumes that it needs to be converted to milliseconds,
	 * so it sets the max value to MAX_VALUE/1000
	 */
	...
	private class InactivityMonitor extends Thread {
		...
		public void run() {
			...
			synchronized (subscriber.getFileLock()) {
				while (keepRunning) {
					try {
						trace.trace("InactivityMonitor - Waiting for " + (fileInactiveSaveInterval / 1000)
								+ " seconds of inactivity.", TraceLevel.TRACE);
						subscriber.getFileLock().wait(fileInactiveSaveInterval);
						trace.trace("InactivityMonitor - Awake after " + (fileInactiveSaveInterval / 1000)
								+ " seconds of inactivity.", TraceLevel.TRACE);
						if (keepRunning && subscriber.finishFile()) {
							trace.trace("InactivityMonitor - Closed file after " + (fileInactiveSaveInterval / 1000)
									+ " seconds of inactivity.", TraceLevel.TRACE);
						}
					} catch (final InterruptedException e) {
						trace.trace("InactivityMonitor - Interupted", TraceLevel.TRACE);
					} catch (final WriteException e) {
						trace.trace("InactivityMonitor - Unable to close file:" + e.getMessage(),
	...
	private class MaxFileAgeMonitor extends Thread {
		...
		public void run() {
			...
			synchronized (subscriber.getFileLock()) {
				try {
					trace.trace("MaxFileAgeMonitor - Waiting for " + (maxFileAge / 1000) + " seconds after cration.",
							TraceLevel.TRACE);
					subscriber.getFileLock().wait(maxFileAge);
					trace.trace("MaxFileAgeMonitor - Awake after " + (maxFileAge / 1000) + " seconds after cration.",
							TraceLevel.TRACE);
					if (subscriber.finishFile()) {
						trace.trace("MaxFileAgeMonitor - Closed file after " + (maxFileAge / 1000)
								+ " seconds of creation.", TraceLevel.TRACE);
					}
				} catch (final InterruptedException e) {
	...
	private org.quartz.Scheduler cronScheduler = null;
	private Trace trace;
	 ...
	 *
	 * @see
	 * info.vancauwenberge.filedriver.api.IStrategy#init(com.novell.nds.dirxml.
	 * driver.Trace, java.util.Map, info.vancauwenberge.filedriver.api.IDriver)
	 */
	@Override
	public void init(final Trace trace, final Map<String, Parameter> driverParams, final IDriver driver)
		...
		// Integer.parseInt(driverParams.get(NEW_FILE_MAX_FILE_AGE).toString())
		// * 1000;
		fileInactiveSaveInterval = getIntValueFor(Parameters.NEW_FILE_INACTIVE_SAVE_INTERVAL, driverParams) * 1000;
		;
		// Integer.parseInt(driverParams.get(NEW_FILE_INACTIVE_SAVE_INTERVAL).toString())
		// * 1000;
		...
		// and if the interval is smaller then the 'live' time of a file
		// (fileSaveInterval)
		if (fileInactiveSaveInterval > 0) {
			if ((maxFileAge == 0) | (fileInactiveSaveInterval < maxFileAge)) {
				...
				trace.trace(
						"Warn: file inactivity monitor set but not used since fileInactiveSaveInterval("
								+ fileInactiveSaveInterval + ") > maxFileAge(" + maxFileAge + ")",
						TraceLevel.ERROR_WARN);
			}
		}
		...
		if (trace.getTraceLevel() >= TraceLevel.TRACE) {
			...
			trace.trace(" MaxRecordCount:" + maxRecordCount + ((maxRecordCount == 0) ? "(disabled)" : "(enabled)"),
					TraceLevel.TRACE);
			trace.trace(" maxFileAge:" + maxFileAge + ((maxFileAge == 0) ? "(disabled)" : "(enabled)"),
					TraceLevel.TRACE);
			trace.trace(" fileInactiveSaveInterval:" + fileInactiveSaveInterval
					+ ((fileInactivityMonitor == null) ? "(disabled)" : "(enabled)"), TraceLevel.TRACE);
	...
	private void initQuartz() throws Exception {
		if (cronScheduler == null) {
			// Do the test here, otherwise quartz will be imported.
			if ((cronExpression != null) && !"".equals(cronExpression.trim())) {
				cronScheduler = QuartzUtil.initQuartz(trace, cronExpression.trim(), subscriber, this);
			} else {
	...
	/**
	 * Reschedule the maximum file age monitor (stop the old monitor and start a
	 * new one).
	 */
	/*
	 * private void rescheduleMaxAgeMonitor(){ //Only do something if
	 * fileInactiveSaveInterval is set (>0) // and if the interval is smaller
	 * then the 'live' time of a file (fileSaveInterval) if (maxFileAge>0){ if
	 * (maxFileAgeMonitor!=null) //We had one before. Finish it.
	 * maxFileAgeMonitor.interrupt(); else { maxFileAgeMonitor = new Thread(){
	 * public void run(){ synchronized(subscriber.getFileLock()){ while (true){
	 * try { subscriber.getFileLock().wait(maxFileAge); if
	 * (subscriber.finishFile()) trace.trace("Closed file "+(maxFileAge/1000)+
	 * " seconds after creation.",TraceLevel.TRACE); } catch
	 * (InterruptedException e) { trace.trace(getName()+ " Interupted"
	 * ,TraceLevel.TRACE); } catch (WriteException e) { trace.trace(
	 * "Unable to close file:"+e.getMessage()); } } } } };
	 ...
	 */
	/**
	 * Reschedule the inactivity monitor (stop the old monitor and start a new
	 * one).
	 */
	/*
	 * private void rescheduleInactivityMonitor(){ //Only do something if
	 * fileInactiveSaveInterval is set (>0) // and if the interval is smaller
	 * then the 'live' time of a file (fileSaveInterval) if
	 * ((fileInactiveSaveInterval>0) & (fileInactiveSaveInterval<maxFileAge)){
	 * if (fileInactivityMonitor!=null) //We had one before. Finish it.
	 * fileInactivityMonitor.interrupt(); else{ //Save the file after nnn
	 * seconds of inactivity fileInactivityMonitor = new Thread(){ public void
	 * run(){ synchronized(subscriber.getFileLock()){ while(true){ try {
	 * subscriber.getFileLock().wait(fileInactiveSaveInterval); if
	 * (subscriber.finishFile()) trace.trace("Closed file after "
	 * +(fileInactiveSaveInterval/1000)+" seconds of inactivity."
	 * ,TraceLevel.TRACE); } catch (InterruptedException e) {
	 * trace.trace(getName()+ " Interupted",TraceLevel.TRACE); } catch
	 * (WriteException e) { trace.trace("Unable to close file:"
	 * +e.getMessage(),TraceLevel.ERROR_WARN); } } } }
	 *
	 ...
	 *
	 * @see
	 * info.vancauwenberge.filedriver.api.INewFileDecider#requiresNewFile(com.
	 * novell.nds.dirxml.driver.Trace, java.util.Map) Returns true when nnn
	 * records have been saved in the file (nnn=value of parameter
	 * NEW_FILE_MAX_RECORDS)
	 */
	@Override
	public boolean requiresNewFile(final Map<String, String> record) {
		...
		synchronized (subscriber.getFileLock()) {
			if (subscriber.getCurrentFileRecordCount() <= 0) {// always return
																// true when no
																// records have
																// been saved
																// yet!
				...
				if (value != null) {// If we have a value, use that algorithm
					if ("true".equals(value)) {
						...
						trace.trace("Not closing file after explicit setting of '" + fieldName + "' to NOT 'true'.",
								TraceLevel.TRACE);
						return false;
					}
				} else {// We do not have a value, use the maxFileRecords
						// algorithm (if not zero)
					if ((maxRecordCount != 0) && (subscriber.getCurrentFileRecordCount() >= maxRecordCount)) {
						trace.trace("Closing file after " + maxRecordCount + " records have been added.",
								TraceLevel.TRACE);
						return true;
					} else {
	...
	public void onShutdown(final XDSResultDocument reasonXML) {
		...
		if (cronScheduler != null) {
			try {
				...
				final StatusAttributes attrs = StatusAttributes.factory(StatusLevel.FATAL, StatusType.DRIVER_STATUS,
						null); // event-id
	 ...
	 * (non-Javadoc)
	 *
	 * @see info.vancauwenberge.filedriver.api.ISubscriberFileListener#
	 * afterFileOpened(info.vancauwenberge.filedriver.api.
	 * ISubscriberFileListener.FileEvent)
	 */
	@Override
	 ...
	 *
	 * @see
	 * info.vancauwenberge.filedriver.api.ISubscriberFileListener#afterFileClose
	 * (info.vancauwenberge.filedriver.api.ISubscriberFileListener.FileEvent)
	 */
	@Override
	 ...
	 *
	 * @see info.vancauwenberge.filedriver.api.ISubscriberFileListener#
	 * afterRecordAdded(info.vancauwenberge.filedriver.api.
	 * ISubscriberFileListener.FileEvent)
	 */
	@Override
