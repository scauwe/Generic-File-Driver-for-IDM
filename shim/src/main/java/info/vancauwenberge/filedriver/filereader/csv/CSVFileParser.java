...
public class CSVFileParser {
	...
	private final Object fileHeaderprocessedLock = new Object();
	...
	public void resetParser() {
		synchronized (fileHeaderprocessedLock) {
			if (hasHeader) {
				fileHeaderprocessed = false;
			} else {
				fileHeaderprocessed = true;
			}
			fileHeaderprocessedLock.notifyAll();
		}
	...
	public void doParse(final Reader stream) throws java.text.ParseException, IOException {
		...
		// if the file does not finish with a newline, save the last field and
		// record
	...
	private void saveField(final StringBuffer aPart) {
		...
		// If we need to load the tags from the header, save it as tag name if
	...
	private int doLineNormal(final char c, final StringBuffer aPart) {
		...
		if (c == token_seperator) {// seperator
			if ((fieldStat == FIELD_NORMAL) || (fieldStat == FIELD_BEGIN) || (charStat == CHAR_ESCAPE)) { // This
																											// ends
																											// the
																											// field
				saveField(aPart);
			...
			if ((fieldStat != FIELD_QUOTED) || ((fieldStat == FIELD_QUOTED) && (charStat == CHAR_ESCAPE))) {
				saveField(aPart);
	...
	/**
	 * Mark the fileheaders as closed and notify any waiting thread that the
	 * status changed
	 */
	private void closeFileHeaders() {
		synchronized (fileHeaderprocessedLock) {
			fileHeaderprocessed = true;
			fileHeaderprocessedLock.notifyAll();
		}
	...
	/**
	 * Should the current record be added as a data record o the queue? returns
	 * true if the record contains data or when skipEmptyLines is false. return
	 * false when skipEmptyLines is true and the record does not contain data.
	 *
	 * @return
	 */
	...
	public CSVFileParser(final char seperator, final String[] dataRecordFields, final boolean skipEmptyLines,
			final boolean hasHeader, final boolean loadNodeNamesFromHeader) {
	...
	public static void main(final String[] args) {
		...
		final CSVFileParser fr = new CSVFileParser(',', new String[] { "customField1", "customField2" }, skipEmptyLines,
				hasHeader, loadNodeNamesFromHeader);

		try {
			// File f = new File();
			StringReader sr = new StringReader(
					"aCSVHeader1,aCSVHeader2\n\nvalue1.1,value1.2\n\n\n\nvalue2.1,value2.2\n\n\n\n\n\n\n");
			fr.resetParser();
			fr.doParse(sr);
